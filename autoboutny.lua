...
swordmethod =getgenv().Setting.Sword.SowrdMode
local mele, gun, sword, fruit
spawn(function()
    while task.wait() do
        pcall(function()
            if swordmethod then
                if getgenv().targ.Character:FindFirstChild("HumanoidRootPart") and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    if (getgenv().targ.Character:WaitForChild("HumanoidRootPart").CFrame.Position - game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart").CFrame.Position).Magnitude < 40 then 
                        if swordmethod and not swordmethod then
                            EquipWeaponTool("Melee")
                            EquipWeaponTool("Gun") 
                        elseif not swordmethod and swordmethod then
                            EquipWeaponTool("Melee")
                            EquipWeaponTool("Sword") 
                        elseif swordmethod and swordmethod then
                            EquipWeaponTool("Melee")
                            EquipWeaponTool("Sword")
                            EquipWeaponTool("Gun") 
                        else
                            ...
                            if getgenv().Setting.Fruit.Enablet then
...
spawn(function()
    ...
    if getgenv().Setting.Another.Bone then
    local args = {
     [1] = "Bones",
     [2] = "Buy",
     [3] = 1,
...
game:GetService("CoreGui").RobloxPromptGui.promptOverlay.ChildAdded:Connect(function(child)
    if not hopserver and child.Name == 'ErrorPrompt' and child:FindFirstChild('MessageArea') and child.MessageArea:FindFirstChild("ErrorFrame") then
        print("jack Hub Auto Bounty | Rejoin!")
...
DropShadow.Image = ""
DropShadow.ImageColor3 = Color3.fromRGB(566,566,899)
...
ImageHub.BackgroundColor3 = Color3.fromRGB(9, 9, 9)
...
Ver.BackgroundColor3 = Color3.fromRGB(455, 655, 566)
